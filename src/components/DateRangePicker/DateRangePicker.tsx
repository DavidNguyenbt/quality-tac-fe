...
import Calendar from "@components/DateRangePicker/Calendar"; // Adjust the import path accordingly
...
const DateRangePicker: React.FC<DateRangePickerProps> = memo(
  ({
    ...
    const label = t?.calendar?.dateRangeLabel ?? "Start date - End date";
    const placeholderText = t?.calendar?.dateRangePlaceholder ?? placeholder;
      ...
      <Box>
        <FormControl sx={{ width: 300 }}>
          ...
          <TextField
            ...
            value={
              inputValue ||
              formatDateRange(selectedDateRange[0], selectedDateRange[1])
            }
            ...
            InputProps={{
              ...
              startAdornment: (
                <CalendarToday
                  sx={{ color: (theme) => theme.color.text.o5, mr: 2 }}
                />
              ),
            }}
            inputProps={{
