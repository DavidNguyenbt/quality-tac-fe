...
import DefectImgItem from './components/DefectImgItem';
...
const PageDashboardFB: React.FC = () => {
    const { id } = useParams();
    ...
    const [lablesdef1, setLabelsdef1] = useState<string[]>([]);
    const [datattpdef1, setDatattpdef1] = useState<number[]>([]);
    const [seriesdef1, setSeriesdef1] = useState<number[]>([]);
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (supp == 'All') {
                ...
                setLabelsdef1(dataName1);
                setDatattpdef1(dataTtp1);
                setSeriesdef1(dataPer1);

                setDatalist1(response.datalist1);
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12} md={7}>
                                    <>
                                        ...
                                        >{facr[0].name} - Fabric inspection RFT % - {yearstart}</Typography>
                                ...
                                <Grid item xs={12} md={5}>
                                    <>
                                        <Typography
                                            align="center"
                                            sx={{ color: (theme) => theme.color.text.o1, fontSize: '14px', fontWeight: 700 }}
                                        >Top 5 Defect Fabric (%) - {monthArray[0].name}</Typography>
                            ...
                            <Grid item xs={12} md={7}>
                                <>
                                    <Typography
                                        align="center"
                                        sx={{ color: (theme) => theme.color.text.o1, fontSize: '14px', fontWeight: 700 }}
                                    >{facr[0].name} - {supp} - {yearstart}</Typography>
                                    <LineBar chartData={series} chartLabel={lables} target1={target1} rft={100} target={100} titleTarget={97} yearstart={yearstart} yearend={yearend} handleChange={handleChangeDate} height={412} />
                                </>
                            </Grid>
                            <Grid item xs={12} md={5}>
                                ...
                                <>
                                    <Grid container spacing={2}>
                                        <Grid item xs={4}>
                                            <Button
                                                variant={title === 'Top 5 Defect' ? 'contained' : 'outlined'}
                                                type="button"
                                                sx={{ width: '100%', color: title === 'Top 5 Defect' ? (theme) => theme.color.neutral.o1 : (theme) => theme.color.text.o6 }}
                                                onClick={() => handleButtonClick('Top 5 Defect')}
                                            >
                                                Top 5 Defect
                                            </Button>
                                        </Grid>
                                        ...
                                        <Grid item xs={4}>
                                            <Button
                                                variant={title === 'Top 3 Item' ? 'contained' : 'outlined'}
                                                type="button"
                                                sx={{ width: '100%', color: title === 'Top 3 Item' ? (theme) => theme.color.neutral.o1 : (theme) => theme.color.background.o6 }}
                                                onClick={() => handleButtonClick('Top 3 Item')}
                                            >
                                                Top 3 Item
                                            </Button>
                                        </Grid>
                                    ...
                                    {title === 'Top 5 Defect' ? (
                                        ...
                                        : <Pie chartData={seriesdef1} chartLabel={lablesdef1} datattpdef={datattpdef1} />}
                            ...
                            <Grid item xs={12} >
                                <Grid container spacing={2}>
                                    {title === 'Top 5 Defect' ? (
                                        ...
                                        : <DefectImgItem datalist={datalist1} />}
...
export default PageDashboardFB;